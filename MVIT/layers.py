...
class MultiHeadSelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        return output, weights,concat_attention
...
class SpatialAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.conv = tf.keras.layers.Conv2D(filters=1, kernel_size=(7, 7), padding='same', activation='sigmoid',use_bias=False)

    def call(self, inputs):

        avgout=tf.reduce_mean(inputs,axis=-1,keepdims=True)
        maxout=tf.reduce_max(inputs,axis=-1,keepdims=True)
        x=tf.concat([avgout,maxout],axis=-1)
        #x=tf.expand_dims(x, axis=0)
        attention=self.conv(x)
        #x=tf.squeeze(attention, axis=0)
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training):
        ...
        x, weights,attention = self.att(x)
