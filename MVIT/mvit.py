...
def build_model(
    ...
    input_matrices = [y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11]
    three_d_matrix=tf.stack(input_matrices,axis=-1)
    conv_layer=tf.keras.layers.Conv2D(
        filters=1,
        kernel_size=(1,1),
        padding='same'
    )
    output_matrix=conv_layer(tf.expand_dims(three_d_matrix,axis=0))
    y_out=tf.squeeze(output_matrix,axis=0)
    y_out = tf.squeeze(y_out, axis=-1)
    print(y)
    print(y_out)
    y=y+y_out
